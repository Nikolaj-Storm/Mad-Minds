...
- **Self-hosted connector changes** (`meta-ads-mcp/`, `gads-mcp/`, `gsc-mcp/`) → redeploy the server (not a plugin version bump). Meta: `mcp-stack/` on the Hetzner box (`git pull && docker compose up -d --build`); GSC/Google Ads: Fly. Keep `meta-ads-mcp` analysis+write parity with `gads-mcp`, and never weaken the `READONLY_MODE` / `/ad-actions` gating.
