...
The plugins read from and write to the shared **Mad Minds** Google Drive Hub. Every connector is **per-user OAuth**. Google Drive is in Claude desktop's built-in catalog; **Google Search Console, Google Ads, and Meta Ads are self-hosted MCPs in this repo** (`gsc-mcp/`, `gads-mcp/`, `meta-ads-mcp/`), added as custom connectors — Meta runs as **two connectors**, one per business area (onlineminds.io + Rentumo ApS). Plus pre-wired vendor MCPs (Notion, Supabase, Vercel, Slack — in `onlineminds-marketing/.mcp.json`). GA4 / Tag Manager / Merchant Center aren't wired yet, and **Composio is not used** (an earlier draft did; removed).
...
- **Self-hosted connector changes** (`meta-ads-mcp/`, `gads-mcp/`, `gsc-mcp/`) → redeploy the server (not a plugin version bump). All three now run on the **Hetzner box** (`mcp@37.27.23.202`, "Openclaw", rootless Docker) as Docker Compose services behind **Tailscale Funnel**: Meta is `mcp-stack/compose.yaml` (project `madminds-mcp`, hosts `meta-onlineminds`/`meta-rentumo`); Google Ads + GSC are `mcp-stack/compose.google.yaml` (project `madminds-google`, hosts `gads`/`gsc`, each its own container + tunnel sidecar, disk-backed token storage at `CLIENT_STORAGE_DIR=/data`). Redeploy with `cd ~/Mad-Minds/mcp-stack && git pull && docker compose -f <file> up -d --build` — use the right `-f`/project so you don't recreate the others. Each server also ships a `Dockerfile`, a `vercel.json` + `api/index.py` (Vercel serverless alternative, Redis/KV token storage), and a legacy `fly.toml`. Keep `meta-ads-mcp` analysis+write parity with `gads-mcp`, and never weaken the `READONLY_MODE` / `/ad-actions` gating. If a connector's public URL changes, bump the plugin version and update the `_custom_connectors_note` in `onlineminds-marketing/.mcp.json` + CONNECTORS.md so marketers get the new URL.
...
- Self-hosted connector deploy: `META-SELF-HOST-RUNBOOK.md` + `mcp-stack/README.md` (Meta); `GADS-SELF-HOST-RUNBOOK.md`, `GSC-SELF-HOST-RUNBOOK.md`
