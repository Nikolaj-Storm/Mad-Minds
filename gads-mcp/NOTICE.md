...
1. **Auth → per-user OAuth proxy.** `client.py:get_client()` no longer loads a static `google-ads.yaml` refresh token. It builds a `GoogleAdsClient` per request from:
   ...
   - a server-side **developer token** (`GOOGLE_ADS_DEVELOPER_TOKEN`) — one for the whole org, set in the server's env file, and
...
## Safety
- `READONLY_MODE` (env var, default **"true"**) makes all write tools **simulate** — they report what they would change without touching Google Ads. Flip to `"false"` only when you're ready for real writes.
