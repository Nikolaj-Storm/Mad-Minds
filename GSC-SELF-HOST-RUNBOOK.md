...
**How it works (the important part):** we deploy a small server that acts as the **OAuth broker**. It holds the Google client secret *server-side* and presents a standard Connect handshake to Claude — the same pattern Pipeboard uses for Google/Meta Ads. That's what makes it zero-setup for colleagues. (The earlier draft used a server that delegated auth to Google, which forced each marketer to paste a Client ID/Secret — this version removes that.)
