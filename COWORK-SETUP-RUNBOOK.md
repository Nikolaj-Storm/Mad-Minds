...
4. **Pre-wired vendor MCPs** in `onlineminds-marketing/.mcp.json` (Ahrefs, SimilarWeb, Notion, Supabase, Vercel, Slack) load automatically when the plugin installs. Each marketer authorizes those on first use (API key for Ahrefs/SimilarWeb, vendor OAuth for the rest).
