...
| Rentumo Conversions | Self-hosted Hetzner MCP (SSE, Tailscale Funnel) | **None** — public S3 feeds, no bearer/OAuth (pre-wired — no marketer action needed) | **Rentumo brand only** (not the other brands). **Read-only** Rentumo conversion + UTM-attribution data per market, from two public feeds: a first/last-touch attribution feed (counts by `utm_source`/`medium`/`campaign`/`content`/`term` — where channels like **Lifull-connect** appear) and a Google Ads offline-conversions feed (counts **+ value**, per-market currency, plus the new-sub vs renewal split). **Emails in the source are hashed and never returned — output is always aggregated.** Money is per-market currency — never sum value across markets. Tools: `conversions_list_markets`, `conversions_get_attribution`, `conversions_get_offline_summary`, `conversions_get_source`. No write path (so no spend-gate surface). See `rentumo-conversions-mcp/README.md` + `RENTUMO-CONVERSIONS-SELF-HOST-RUNBOOK.md`. |
