...
| Subscribers — Rentumo | Rentumo Trials (plugin MCP, shared bearer token) | Read-only new-subscriber (trial) counts across all Rentumo markets (`rentumo_list_markets`, `rentumo_get_trials`, `rentumo_get_all_trials`). Pre-wired — no per-user auth. The new-subscriber outcome metric for Rentumo: pair it with Google/Meta/Thribee spend to read cost-per-new-subscriber. Pass ISO dates; `rentumo_get_all_trials` returns the portfolio total. Rentumo only. |
