...
2. **`list_accounts` returns the queryable client accounts.** It walks the manager tree and returns each client account with its `customer_id` and the `login_customer_id` (the manager to send as the login-customer-id header). Always pass BOTH to `get_performance`, `get_campaigns`, `get_search_terms`, `get_conversion_lag`, etc. when working a client account under a manager.

**Days to conversion / conversion lag.** `get_performance` has no segment breakdown. For "how many days between click and conversion", a cumulative "% landed within N days" curve, or judging whether a recent week's numbers are mature enough to compare — use `get_conversion_lag` (queries `segments.conversion_lag_bucket`, matching the Google Ads UI's Segment > Conversions > "Days to conversion" view). Its date window filters the **ad-click date, not the conversion date**, so a very recent window will show low percentages simply because conversions haven't landed yet — that's expected. Rentumo has one Google Ads account **per market**, so call it once per market's `customer_id` for a per-market breakdown.
