...
- **Meta Ads** (custom connector — self-hosted, like Google Ads) — Facebook/Instagram reporting + management (read+write); per-user; they see only ad accounts they can access across every Business Manager they belong to; writes simulate (READONLY_MODE) until enabled and route through `/ad-actions`. **One org-wide URL — no per-company minting.** Steps: Customize → Connectors → **Add custom connector** → URL `https://<meta-mcp-domain>/mcp` → leave Advanced settings empty → **Add** → **Connect** → sign in with their **Facebook** account. (After connecting, a new session may be needed for the tools to appear. The marketer's Facebook must hold a role in the relevant ad accounts; if Meta returns a permission error, they're not added to that account/Business Manager yet.)
