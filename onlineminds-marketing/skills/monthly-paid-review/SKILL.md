...
4. **Thribee live pull:** call `thribee_get_spend` (or `thribee_get_all_spend` for portfolio reviews) for the month and prior month. Thribee reports each market's spend in a **fixed per-market currency** — use the authoritative table in account-conventions ("Thribee spend currency (per market)"): most markets are **EUR** (incl. SE, PL, CH, CZ, HU, RO, MX — *not* their local currency), with UK=GBP, DK=DKK, BR=BRL, and AU/CA/MY=USD. Convert each market's figure to the house reporting currency before blending, state the source currency, and never sum across currencies. Include in the blended and per-channel views.
5. **Rentumo subscribers + revenue (Rentumo only):** call `rentumo_get_all_trials` (or `rentumo_get_trials` for a single market) for the month and prior month, passing ISO dates. Each market returns both the new-subscriber (trial) count and **gross revenue** (`revenue_gross`), plus chargeback fields. Report:
   - **Cost-per-new-subscriber** — total spend / `total_new_subscriptions` (blended; the headline efficiency metric), with per-market breakdown and vs prior month.
   - **Revenue and blended MER** — `revenue_gross` per market and revenue / spend as a blended MER (not a platform ROAS — this is total gross admin revenue, not ad-attributed). **Currency:** `revenue_gross` is in each market's local currency, so convert each market to the house reporting currency before combining; never sum raw `revenue_gross` across markets (`rentumo_get_all_trials` deliberately does not). Net of `chargeback_money_lost` where it materially affects the picture.
   Skip for non-Rentumo brands.
